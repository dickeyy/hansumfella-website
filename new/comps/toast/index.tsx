...
export default function Toast({ title, type, id, duration }: { title: string, type: string, id: string, duration: number }) {
    ...
    if (type === 'success') {
        ...
        return (
            <AnimatePresence>
                {isVis && (
                    <motion.div id={id} className="flex items-center w-full max-w-xs p-4 mb-4 text-white bg-red-500  rounded-[12px] shadow dark:text-gray-400 z-50 absolute bottom-0" role="alert"
                        initial={{
                            y: 10,
                            opacity: 0
                        }}
